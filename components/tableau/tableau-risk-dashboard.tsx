  ...
  const metrics = useMemo((): RiskMetrics => {
    ...
    holdings.forEach((holding) => {
      ...
      if (ownershipPercent >= threshold) {
        ...
        // Use totalExposure instead of sharesOwned for accurate calculation
...
function TopRiskPositions({ holdings }: { holdings: Holding[] }) {
  const riskPositions = useMemo(() => {
    // Aggregate holdings by ticker + jurisdiction to avoid duplicates
    // This ensures we show one entry per unique ticker+jurisdiction combination
    const aggregatedMap = new Map<string, {
      ...
      totalBuyingVelocity: number;
      regulatoryRule: typeof holdings[0]['regulatoryRule'];
    }>();

    holdings.forEach((holding) => {
      // Create a unique key for ticker + jurisdiction combination
      const key = `${holding.ticker}-${holding.jurisdiction}`;
      ...
      if (!aggregatedMap.has(key)) {
        aggregatedMap.set(key, {
          ...
          totalSharesOwned: holding.sharesOwned,
          totalSharesOutstanding: holding.totalSharesOutstanding,
          totalBuyingVelocity: holding.buyingVelocity,
          regulatoryRule: holding.regulatoryRule,
        });
      } else {
        const aggregated = aggregatedMap.get(key)!;
        aggregated.holdings.push(holding);
        aggregated.totalSharesOwned += holding.sharesOwned;
        aggregated.totalBuyingVelocity += holding.buyingVelocity;
        // Use the most recent holding for metadata
        // Note: totalSharesOutstanding should be the same for same ticker, but we use the most recent
        const mostRecent = aggregated.holdings.reduce((latest, h) => 
          new Date(h.lastUpdated) > new Date(latest.lastUpdated) ? h : latest
        );
        aggregated.issuer = mostRecent.issuer;
        // For same ticker, totalSharesOutstanding should be consistent, but use most recent
        aggregated.totalSharesOutstanding = mostRecent.totalSharesOutstanding;
        aggregated.regulatoryRule = mostRecent.regulatoryRule;
      }
    });
    ...
    return Array.from(aggregatedMap.values())
      .map((aggregated) => {
        // Calculate delta-adjusted exposure for aggregated holdings
        // Sum up delta-adjusted exposures from all holdings for this ticker
        const totalAggregatedExposure = aggregated.holdings.reduce((sum, h) => {
          return sum + calculateDeltaAdjustedExposure(h);
        }, 0);
        const ownershipPercent =
          (totalAggregatedExposure / aggregated.totalSharesOutstanding) * 100;
        const threshold = aggregated.regulatoryRule.threshold;
        ...
        if (ownershipPercent >= threshold) {
          ...
          status = "warning";
          if (aggregated.totalBuyingVelocity > 0) {
            const thresholdShares = (threshold / 100) * aggregated.totalSharesOutstanding;
            const sharesToBreach = thresholdShares - totalAggregatedExposure;
            const hoursToBreach = sharesToBreach / aggregated.totalBuyingVelocity;
            projectedBreachTime = hoursToBreach;
          }
        }
        ...
        const compliance = {
          ...
          requiredForm: aggregated.regulatoryRule.name,
          deadline: "N/A",
          deadlineDays: null as number | null,
        };
        
        // Create a synthetic holding for display purposes
        const displayHolding: Holding = {
          id: `${aggregated.ticker}-${aggregated.jurisdiction}`,
          ticker: aggregated.ticker,
          issuer: aggregated.issuer,
          isin: aggregated.holdings[0].isin,
          jurisdiction: aggregated.jurisdiction,
          sharesOwned: aggregated.totalSharesOwned,
          totalSharesOutstanding: aggregated.totalSharesOutstanding,
          buyingVelocity: aggregated.totalBuyingVelocity,
          regulatoryRule: aggregated.regulatoryRule,
          lastUpdated: aggregated.holdings.reduce((latest, h) => 
            new Date(h.lastUpdated) > new Date(latest.lastUpdated) ? h : latest
          ).lastUpdated,
        };
      ...
      .sort((a, b) => {
        // Prioritize breaches over warnings
        if (a.breach!.status === "breach" && b.breach!.status !== "breach")
          return -1;
        if (a.breach!.status !== "breach" && b.breach!.status === "breach")
          return 1;
        // For same status, sort by time to breach (most urgent first)
        // Breaches with no time get priority, then warnings by projected time
        if (a.breach!.status === "breach" && b.breach!.status === "breach") {
          return 0; // All breaches are equally urgent
        }
        return (
