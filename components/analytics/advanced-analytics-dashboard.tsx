...
export default function AdvancedAnalyticsDashboard() {
  ...
  const metrics = useMemo(() => {
    ...
    // OPTIMIZED: Process holdings in smaller batches to avoid blocking
    // Calculate risk status for each holding
    const holdingsWithStatus: Array<Holding & { ownershipPercent: number; status: "breach" | "warning" | "safe"; timeToBreach: number | null }> = [];
    ...
    for (let i = 0; i < holdings.length; i += BATCH_SIZE) {
      ...
      const batchResults = batch.map((holding) => {
      ...
      holdingsWithStatus.push(...batchResults);
    }
