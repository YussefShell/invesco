  ...
  const { filtered, effectiveAsOf } = useMemo(() => {
    ...
    if (tickerFilter.trim()) {
      ...
      result = result.filter((line) => {
        ...
        return ticker && ticker.toUpperCase().includes(searchTerm);
      });
    }
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="max-h-64 overflow-y-auto rounded border border-border bg-muted/40 px-3 py-2 font-mono text-[11px]">
          {filtered.length === 0 ? (
            ...
            filtered.map((line, idx) => (
              <div key={idx} className="whitespace-pre leading-relaxed py-1 border-b border-border/50 last:border-0">
                {line}
              </div>
            ))
          )}
