...
export default function PreTradeSimulator({
  ...
  const calculation = useMemo(() => {
    ...
    let portfolioData: { currentShares: number; totalOutstanding: number };
    
    if (holding) {
      // Use real-time data from portfolio
      portfolioData = {
        currentShares: holding.sharesOwned,
        totalOutstanding: holding.totalSharesOutstanding,
      };
    } else {
      // Fallback to static data for tickers not in portfolio
      portfolioData = FALLBACK_PORTFOLIO_DATA[tickerUpper] || {
        currentShares: 1000000,
        totalOutstanding: 1000000000,
      };
    }

    const currentOwnership = (portfolioData.currentShares / portfolioData.totalOutstanding) * 100;
    const newTotalShares = portfolioData.currentShares + quantity;
    const resultingOwnership = (newTotalShares / portfolioData.totalOutstanding) * 100;
